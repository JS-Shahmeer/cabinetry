...
export default function CraftsmanshipSection({ image }) {
  return (
    <section id="craftsmanship" className="bg-primary px-5 py-12 text-primary-foreground lg:px-8 md:py-16">
      <div className="mx-auto grid max-w-7xl gap-14 lg:grid-cols-2">
        <div>
          ...
          <div className="mt-10 space-y-3">{features.map((item, i) => <div key={item} className="flex gap-5 border-t border-white/15 py-2 items-center"><span className="font-display text-2xl text-[#C8A977]">0{i + 1}</span><p className="text-lg text-white/82">{item}</p></div>)}</div>
        </div>
        <div>
          <img src={image} alt="Close up of solid wood cabinetry joinery and premium finish" className="h-[560px] md:h-[600px] w-full object-cover" />
